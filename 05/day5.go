...
func parseInput(fpath string) (Input, error) {
	var input Input
	...
	input.rules = parseRules(sections[0])
	input.updates = parseUpdates(sections[1])
	return input, nil
}
...
func parseUpdates(content string) []Update {
	updates := []Update{}
	for _, line := range strings.Split(content, "\n") {
		if line == "" {
			continue
...
func (u Update) isCorrectOrder(rules []Rule) bool {
	var totalFailed int
	for _, rule := range rules {
		if u.incorrectOrder(rule) {
			fmt.Printf("%v update failed due to rule %v\n", u, rule)
			totalFailed++
		}
	}
...
func (r Rule) isValid(update Update) bool {
	// determine if the rule concerns pages present in the update
	var r0, r1 bool // bools default to false
...
func (u Update) incorrectOrder(rule Rule) bool {
	// if rule contains page not present in the update, then ignore -> return true
	if !rule.isValid(u) {
		return false
	}
	var idx1, idx2 int
...
func fixIncorrectUpdateOrder(u Update, rule Rule) Update {
	var idx1, idx2 int
	for idx, val := range u {
		if val == rule[0] {
			idx1 = idx
	...
	if idx1 < idx2 {
		...
		// flip values at idx1, idx2
		// TODO: make this
	}
	return u
}

func (u Update) getMiddlePageNum() int {
	return u[len(u)/2]
}
...
func part1(input Input) int {
	...
	for _, update := range input.updates {
		if update.isCorrectOrder(input.rules) {
			fmt.Printf("%v update passed\n", update)
			total += update.getMiddlePageNum()
		}
	}
	return total
}
